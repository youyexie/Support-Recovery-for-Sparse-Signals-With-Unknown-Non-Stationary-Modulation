...
PT = zeros(numIter,length(2:0.5:xlimit));


    for x = 2:0.5:xlimit
        %fprintf('eta = %d\n',eta);
        for Iter = 1:numIter
            fprintf('x = %d, Iter = %d',x,Iter);
            % Dictionary A
            ...
            deltaX = max( norms( X-X0_scaled,2,1) );
            fprintf(', error = %f',deltaX);
...
scatter(1:length(2:0.5:xlimit),meanPT,200,'+','r','LineWidth',3);
...
xlabel('k from 2 to 6') 
ylabel('Recovery error')
grid on
set(gca,'FontSize',20)
hold off
%%
%save('RecoveryError-x.mat','PT');
