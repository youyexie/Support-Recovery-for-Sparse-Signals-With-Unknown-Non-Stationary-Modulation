...
for K = 1:20
    for N = 30:5:100
        %fprintf('eta = %d\n',eta);
        for Iter = 1:numIter
            fprintf('N = %d, K = %d, Iter = %d',N,K,Iter);
            % Dictionary A
            ...
            if (~ismember(0, ismember(find(norms( X, 2, 1 )>1e-5 ) , find((norms(X0_scaled,2,1)>1e-5)) ) ))
                ...
                if (sum(norms(X))>1e-5)
                    PT_NoZeroSolution((N-30)/5+1,K) = PT_NoZeroSolution((N-30)/5+1,K) + 1;
                end
                ...
                if (length( find(norms( X, 2, 1 )>1e-5 )) == length( find((norms(X0_scaled,2,1)>1e-5)))  ) && min(  find(norms( X, 2, 1 )>1e-5 )==find((norms(X0_scaled,2,1)>1e-5)) )
                    PT_ExactRecovery((N-30)/5+1,K) = PT_ExactRecovery((N-30)/5+1,K) + 1;
                end
            else
...
%%
%save('K.mat','PT');
%save('K(0NotCount).mat','PT_NoZeroSolution');
%save('K(ExactRecovery).mat','PT_ExactRecovery');